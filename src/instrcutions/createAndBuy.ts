...
export const createAndBuyTx = async(tokenmeatda:Metadata,metadat_uri:string,solBuyAmount:number,payer:Keypair,mint:Keypair) => {

    const instructions:TransactionInstruction[] = [];

    const createItx = await createBonkTokenTx(tokenmeatda,metadat_uri,solBuyAmount,payer,mint);
    
    instructions.push(...createItx);

    const butItx = await makeBuyIx(payer,0.05*LAMPORTS_PER_SOL,mint.publicKey);
